...
import { SprechtrainerService } from '@/services/sprechtrainerService';
...
describe('SprechtrainerService', () => {
  test('builds a speaking application note for speaking-oriented Lernauftrag work', () => {
    const plan = buildSessionPlan(profile, { notesFolder: 'neurolex/' });
    const sprechtrainer = new SprechtrainerService('neurolex/');
    const artifact = sprechtrainer.buildSpeakingArtifact(profile, plan, new Date('2026-04-02T12:00:00.000Z'));

    expect(plan.applicationMode).toBe('speaking');
    expect(artifact.kind).toBe('speaking-application');
    expect(artifact.path).toBe('neurolex/sessions/2026-04/speaking-application-2026-04-02T12-00-00.000Z.md');
    expect(artifact.content).toContain('# NeuroLex Speaking Application');
